...
exports.handler = async event => {
    ...
    const { year, month, day, sensorType } = event.pathParameters;

    if (!(sensorType in SENSOR_TYPES)) {
        return Responses._400({ error: true, message: 'Invalid sensor type' });
    }
    try {
        items = await Dynamo.queryDocumentsSkBegins(`ACCOUNT#${accountName}`, `#${SENSOR_TYPES[sensorType]}#${year}-${month}-${day}`);
