...
exports.handler = async event => {
    ...
    try {
        items = await Dynamo.queryDocumentsSkBeginsAndParams(`ACCOUNT#${accountName}`, `#${SENSOR_TYPES[sensorType]}#`, params);

        return Responses._200({ success: true, data: items.length ? items[0] : null });
    } catch (error) {
        console.log(error);
