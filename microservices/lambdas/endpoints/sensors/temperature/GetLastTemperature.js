...
exports.handler = async event => {
    try {
        ...
        items = await Dynamo.queryDocumentsSkBeginsAndParams(`ACCOUNT#${accountName}`, '#TEMPERATURE#', params);

        return Responses._200({ success: true, temperature: items.length ? items[0] : null });
    } catch (error) {
        console.log(error);
